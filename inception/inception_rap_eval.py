...
from inception import inception_rap_model as inception
...
def softmax_or_not(predict, is_softmax=False, threshold=[0.5]*51):
  res = []
...
def save_to_file(attrs, labels):
  file = open('rap_final.txt', 'w')
  attrs = attrs.astype(int)
  labels = labels.astype(int)
  for i in range(len(attrs)):
    file.write(" ".join('%s' % id for id in attrs[i]))
    file.write("---")
    file.write(" ".join('%s' % id for id in labels[i]))
    file.write("\n")
...
def _eval_once_mA(saver, mA_op, summary_writer, summary_op):
  with tf.Session() as sess:
    ...
    try:
      ...
      attrs = np.zeros((total_sample_count , 51))
      labels = np.zeros((total_sample_count, 51))
      step = 0
      ...
      while step < num_iter and not coord.should_stop():
        ...
        # threshold= [ -1.63896918, -2.92778826, -0.85942656, -0.18633965, -2.28389955,  0.64350349,
        #              -2.49138522,  1.69379652, -2.7792089,  -2.94575572, -2.16644192,  1.65624774,
        #              -2.90464497, -2.6456902,  -2.91939425, -1.9960742,  -2.35979962, -2.7363832,
        #              -1.86692214, -2.19749665, -1.53251255, -2.86584759, -2.82985973, -2.89830303,
        #              -0.28588042, -2.70379686, -2.70369697, -2.70372081, -1.78928852, -2.40788651,
        #              -1.56871521, -1.63923609, -2.46088076, -2.8779881,  -2.09501028, -2.89457226,
        #              -2.69188547, -2.85242891, -2.77874351, -2.84279823, -2.90740204, -2.87133193,
        #              -1.38525283, -2.83333468, -2.78677249, -2.47449708, -2.84807682, -2.92042303,
        #              -2.9037981 , -2.86978507, -2.39694023]
        # threshold = [x / 2 for x in threshold]
        threshold = [0.0] * 51
