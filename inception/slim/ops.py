...
# @scopes.add_arg_scope
# def roi_pool(inputs, rois, height, width, stride, scope=None, restore=True):
#   """Adds a ROI Pooling layer.
...
#   Args:
#     inputs: a tensor of size [batch_size, height, width, depth].
#     rois: an array contains roi heights, weights
#     stride: a list of length 2: [stride_height, stride_width].
#       Can be an int if both strides are the same.  Note that presently
#       both strides must have the same value.
#     scope: Optional scope for name_scope.
#     restore: True : use base model parameters; False : not use

#   Returns:
#     a tensor representing the results of the pooling operation.
#   """
#   with tf.name_scope(scope, 'ROIPool', [inputs]):
#     return roi_pooling(inputs, rois, height, width)

@scopes.add_arg_scope
def roi_avg_pool(inputs, rois, stride, scope=None, restore=True):
  """Adds a roi Avg Pooling layer. pooled_w = pooled_h = 1
  ...
  Args:
    inputs: a tensor of size [batch_size, height, width, depth].
    rois: roi for pooling,size [batch_size, 4] [ymin, xmin, ymax, xmax]
    scope: Optional scope for name_scope.

  Returns:
    a tensor representing the results of the pooling operation.
  """  
  with tf.name_scope(scope, 'ROIAvePool', [inputs]):
    res = []
    rois  = tf.cast(rois, dtype=tf.int32)
    for i in range(inputs.get_shape()[0].value):
      feat = inputs[i]
      roi = []
      roi.append(tf.minimum(tf.floor(tf.divide(rois[i][0], stride)), inputs.get_shape()[2].value-1))
      roi.append(tf.minimum(tf.floor(tf.divide(rois[i][1], stride)), inputs.get_shape()[1].value-1))
      roi.append(tf.minimum(tf.ceil(tf.divide(rois[i][2], stride)), inputs.get_shape()[2].value-1))
      roi.append(tf.minimum(tf.ceil(tf.divide(rois[i][3], stride)), inputs.get_shape()[1].value-1))
      roi  = tf.cast(roi, dtype=tf.int32)
      # roi = tf.Print(roi, [tf.identity(roi)], 'roi= ', summarize=4)
      # inp = tf.slice(feat,  [0, 0,0], [ 1, 1 ,2048])
      inp = tf.slice(feat,  [roi[1], roi[0] ,0], [tf.minimum(roi[3]-roi[1]+1, 8), tf.minimum(roi[2]-roi[0]+1, 8),2048])
      res.append(tf.reduce_mean(tf.reduce_mean(inp, axis=0), axis=0))
      # ave = []
      # for k in range(2048):
      #   tmp = tf.reduce_mean(tf.slice(feat[:,:,k],[0,0], [2,2] ))
      #   ave.append(tmp)
    # res = tf.convert_to_tensor(res, dtype=inputs.dtype.base_dtype, name='poolresult')
    res  = tf.reshape(res, [inputs.get_shape()[0], 2048])
